...
public class WorldTri
{
	...
	public List<WorldTri> extrudeUp( List<WorldVert> verts, float layerH )
	{
		height = layerH;
		...
		if( isUp[0] ){
			...
			if( isUp[1] ){
				...
				Vec3 posNew = c2pos.mult( 1 + (layerH/scale)/c2pos.length() );
			...
			if( isUp[2] ){
				...
				Vec3 posNew = c0pos.mult( 1 + (layerH/scale)/c0pos.length() );
			...
			Vec3 posNew = c1pos.mult( 1 + (layerH/scale)/c1pos.length() );
			...
			for( int c = 0; c < 3; c++ ){
				...
				Vec3 posNew = cPos.mult( 1 + (layerH/scale)/cPos.length() );
		...
		if( child != null ){
			...
			for( int i = 0; i < 3; i++ ){
				if( edge.get(i).twin.midpt != null ){
					...
					Vec3 cPos = ( cI.pos.add( cIp1.pos ).mult(0.5f) ).norm();
	...
	public void set2DCFDEnv( float angle, float velMag, Vec3 upVecWorld ){
		...
		area = pctSurface * mPlate.world.mAreaSurfaceEarth;
		...
		for( int i = 0; i < 3; i++ ){
			...
			corner2D.add( new Vec2( cVec.dot( xLocal_3D ) * scale, cVec.dot( yLocal_3D ) * scale ) );
		}
